...
/**
 * Contains all the logic for Discord login integration.
 */
class DiscordAuthManager extends OAuth2Manager {
   ...
   * The Discord user.
   *
   * @var \League\OAuth2\Client\Provider\DiscordResourceOwner
   */
  protected $user;
  ...
  public function __construct(LoggerChannelFactoryInterface $logger_factory, EventDispatcherInterface $event_dispatcher, EntityFieldManagerInterface $entity_field_manager, UrlGeneratorInterface $url_generator, ConfigFactory $configFactory) {
    ...
    $this->config             = $configFactory->getEditable('social_auth_discord.settings');
  }
   ...
   *
   * @return \League\OAuth2\Client\Provider\DiscordResourceOwner
   *   User info returned by the Discord.
   */
  ...
  /**
   * Returns the Discord login URL where user will be redirected.
   *
   * @return string
   *   Absolute Discord login URL where user will be redirected.
   */
  public function getDiscordLoginUrl() {
    $scopes = [
      'identify',
      'email',
      'connections',
    ...
    $discord_scopes = explode(PHP_EOL, $this->getScopes());
    foreach ($discord_scopes as $scope) {
